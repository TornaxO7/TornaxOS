...
default:
	mkdir -p $(BUILDD)/iso/grub
	cp $(SRCD)/grub.cfg $(BUILDD)/iso/grub
	...
	$(CC) -T $(SRCD)/linker.ld -o $(BUILDD)/iso/kernel.bin -ffreestanding -O2 -nostdlib \
			$(BUILDD)/boot.o $(BUILDD)/kernel.o

	grub-mkrescue -o $(OSNAME).iso $(BUILDD)/iso/kernel.bin
